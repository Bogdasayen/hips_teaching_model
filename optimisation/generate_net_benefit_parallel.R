...
generate_net_benefit_parallel <- function(input_parameters, lambda = 20000) {
  ...
  parallel::mclapply(c(1:n_implants), function(i_implant){
    ...
    for(i_cycle in 2:n_cycles)
    {
      ...
      for(i_state in 1:n_states) {
        cohort_vectors_tr[i_cycle, , i_state] <- 
          rowSums(cohort_vectors_tr[i_cycle - 1 , , ] * transition_matrices_tr[i_cycle - 1, , , i_state])
      }
    } 
  
    
    # Loop below isn't faster in an lapply due to the use of unlist
    #cycle_costs_tr <- matrix(unlist(lapply(c(1:n_samples), function(i){cycle_costs_tr[, i] <- cohort_vectors_tr[, i, ] %*% state_costs_tr[i, ]})), nrow = n_cycles)
    #cycle_qalys_tr <- matrix(unlist(lapply(c(1:n_samples), function(i){cycle_qalys_tr[, i] <- cohort_vectors_tr[, i, ] %*% state_qalys_tr[i, ]})), nrow = n_cycles)
    
    # Also isn't faster after avoiding lapply
    #cycle_costs_tr <- matrix(sapply(c(1:n_samples), function(i){cycle_costs_tr[, i] <- cohort_vectors_tr[, i, ] %*% state_costs_tr[i, ]}), nrow = n_cycles)
    #cycle_qalys_tr <- matrix(sapply(c(1:n_samples), function(i){cycle_qalys_tr[, i] <- cohort_vectors_tr[, i, ] %*% state_qalys_tr[i, ]}), nrow = n_cycles)
  ...
  }, mc.cores = 4) -> output_list # End lapply
